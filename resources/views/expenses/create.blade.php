...
<div class="auth-container" style="max-width: 600px; margin: 2rem auto;">
    <div class="panel">
        ...
        <form action="{{ route('expenses.store', $group) }}" method="POST" id="expense-form">
            @csrf
            
            <div class="form-group">
                <label for="description">Description</label>
                <input type="text" name="description" id="description" class="form-control" placeholder="e.g. Electricity Bill, Dinner at Cafe" required>
            </div>
                    ...
                    @foreach($members as $m)
                        <div class="split-member-row">
                            <label style="display: flex; align-items: center; gap: 0.5rem; margin-bottom: 0; cursor: pointer; user-select: none;">
                                <input type="checkbox" class="participant-check" data-user-id="{{ $m->user->id }}" checked style="width: 16px; height: 16px;">
                                <span>{{ $m->user->name }}</span>
                            </label>
                            
                            <!-- Input for exact/percentage values -->
                            <div class="split-input-wrapper" style="display: none;">
                                <input type="number" name="splits[{{ $m->user->id }}]" class="form-control split-value-input" style="width: 100px; padding: 0.25rem 0.5rem; text-align: right;" step="0.01" min="0" placeholder="0">
            ...
            <div style="display: flex; gap: 1rem; margin-top: 2rem;">
                ...
                <button type="submit" class="btn btn-primary" style="flex: 1;">Log Expense</button>
            </div>
        </form>
    </div>
</div>
...
function adjustSplitInputs() {
    ...
    rows.forEach(row => {
        ...
        if (splitType === 'equal') {
            ...
            // When submitting equal split, we still need to send who is included.
            // We'll set the value to 1 for checked, or empty for unchecked.
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Handle form submit split logic
